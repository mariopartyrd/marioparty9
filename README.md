...
- `config/[RM8E01]` - Configuration files for each game version.
- `config/[RM8E01]/build.sha1` - SHA-1 hashes for each built artifact, for final verification.
- `build/` - Build artifacts generated by the the build process. Ignored by `.gitignore`.
- `orig/[RM8E01]` - Original game files, extracted from the disc. Ignored by `.gitignore`.
- `orig/[RM8E01]/.gitkeep` - Empty checked-in file to ensure the directory is created on clone.
...
- `config/RM8E01/config.example.yml` - Example configuration file and documentation.
